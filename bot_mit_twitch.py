...
# ============================================================
# VOICE CONFIG (EXAKT wie dein Script) :contentReference[oaicite:1]{index=1}
# ============================================================
...
def set_twitch_cfg(
    ...
    cfg["twitch_cooldown_seconds"] = max(0, int(cooldown_seconds))
...
def clear_twitch_cfg(guild_id: int) -> None:
    ...
    for k in [
        ...
        "twitch_stable_checks", "twitch_cooldown_seconds", "twitch_poll_seconds",
        "twitch_last_live_message_id", "twitch_last_check_ts"
    ]:
        cfg.pop(k, None)
    save_config(GUILD_CONFIG)
...
twitch_last_announce_ts: dict[int, float] = {}
...
def parse_twitch_meta(html_text: str) -> dict:
    ...
    meta["is_live"] = bool(re.search(r'"isLiveBroadcast"\s*:\s*true|"isLive"\s*:\s*true', html_text))
...
async def twitch_loop():
    async with aiohttp.ClientSession() as session:
        for guild in bot.guilds:
            ...
            cooldown = int(cfg.get("twitch_cooldown_seconds", 600))
            ...
            prev = twitch_live_state.get(guild.id, False)
            ...
            # OFFLINE -> LIVE
            if not prev and live_now and twitch_live_hits[guild.id] >= stable:
                last_post = twitch_last_announce_ts.get(guild.id, 0.0)
                if cooldown <= 0 or (now - last_post) >= cooldown:
                    twitch_live_state[guild.id] = True
                    twitch_last_announce_ts[guild.id] = now
                    await post_live(guild, cfg, meta)
                else:
                    twitch_live_state[guild.id] = True

            # LIVE -> OFFLINE
            if prev and (not live_now) and twitch_off_hits[guild.id] >= stable:
...
async def on_ready():
    ...
    for g in bot.guilds:
        ...
        twitch_last_announce_ts.setdefault(g.id, 0.0)
...
# ============================================================
# VOICE EVENT (1:1 aus deinem Script) :contentReference[oaicite:2]{index=2}
# ============================================================
@bot.event
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    # !!! WICHTIG: exakt die Keys wie bei dir
...
# ============================================================
# SLASH COMMANDS: VOICE (1:1) :contentReference[oaicite:3]{index=3}
# ============================================================
...
# ============================================================
# SLASH COMMANDS: TWITCH
# ============================================================
@bot.tree.command(name="setup_twitchlive", description="Twitch Live Alerts ohne API: Name oder URL + Textkanal (+ optional Role Ping).")
@app_commands.checks.has_permissions(manage_guild=True)
@app_commands.describe(
    ...
    cooldown_minutes="Anti-Spam: Mindestabstand zwischen LIVE-Posts (empfohlen 10)",
    poll_seconds="Abfragerate in Sekunden (min 30, empfohlen 90)"
)
async def setup_twitchlive(
    interaction: discord.Interaction,
    twitch_channel_or_url: str,
    announce_channel: discord.TextChannel,
    ping_role: discord.Role | None = None,
    stable_checks: app_commands.Range[int, 1, 5] = 2,
    cooldown_minutes: app_commands.Range[int, 0, 120] = 10,
    ...
    twitch_last_announce_ts[gid] = 0.0

    await interaction.response.send_message(
        "✅ Twitch Live-Alerts aktiviert.\n"
        ...
        f"🔇 Stabil: **{stable_checks}** | ⏱️ Cooldown: **{cooldown_minutes} min** | ⏲️ Poll: **{poll_seconds}s**\n"
        f"📌 OFFLINE: **LIVE-Post wird editiert**",
        ephemeral=True
    )
...
async def twitchlive_status(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        ...
        f"🔇 Stable: **{cfg.get('twitch_stable_checks', 2)}** | ⏱️ Cooldown: **{int(cfg.get('twitch_cooldown_seconds', 600))//60} min** | ⏲️ Poll: **{cfg.get('twitch_poll_seconds', 90)}s**\n"
...
async def twitchlive_test(interaction: discord.Interaction):
    cfg = get_guild_cfg(interaction.guild_id) or {}
    if not cfg.get("twitch_enabled"):
        await interaction.response.send_message("ℹ️ Erst /setup_twitchlive ausführen.", ephemeral=True)
        return
